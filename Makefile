...
VFILES:=CT/Algebra/Semigroup.v\
  ...
  CT/Instance/Functor/Presheaf.v\
  ...
  CT/Instance/Category/CommaCategory.v\
  ...
  CT/Instance/Automata/Moore.v\
  CT/Instance/Automata/Moore/Category.v\
...
VO=vo
VOFILES:=$(VFILES:.v=.$(VO))
VOFILES0=$(patsubst CT/%,%,$(filter CT/%,$(VOFILES)))
...
NATIVEFILES0=$(patsubst CT/%,%,$(filter CT/%,$(NATIVEFILES)))
...
install:
	cd "CT" && for i in $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/CT/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/CT/$$i; \
	done

install-doc:
	install -d "$(DSTROOT)"$(COQDOCINSTALL)/CT/html
	for i in html/*; do \
	 install -m 0644 $$i "$(DSTROOT)"$(COQDOCINSTALL)/CT/$$i;\
	done

uninstall_me.sh: Makefile
	echo '#!/bin/sh' > $@
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/CT && rm -f $(NATIVEFILES0) $(GLOBFILES0) $(VFILES0) $(VOFILES0) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "CT" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
